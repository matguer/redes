...
def fuente(map_cant_paquetes, args):
    ...
    if (args.tabla):
        print u"\par Entropía de la fuente: " + "%.2f" % entropia_s + u" bits. Entropía máxima: 32 bits."
    else:
        print u"Entropía (S_1) = " + str(entropia_s) + u" bits. Entropía máxima: 32 bits."

    #info = zip(*info)[1]
    return (info, entropia_s)

def sumarCant(map_cant_paquetes, pkt, args):
    if (pkt.psrc != pkt.pdst and pkt.psrc != "0.0.0.0"):
